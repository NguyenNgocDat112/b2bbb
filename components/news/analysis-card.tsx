...
export function AnalysisCard({ data, priority = false, index = 0 }: AnalysisCardProps) {
  return (
    <Link href={`/news/${data.id}`}>
      ...
      >
        <div className="relative w-full aspect-[1.58] overflow-hidden rounded-[8px] mb-4 shadow-sm group-hover:shadow-[0_12px_24px_rgb(0,0,0,0.12)] transition-all duration-500 bg-gray-100">
          ...
          <Image quality={100}
            ...
            fill
            priority={priority}
            className="object-cover transform transition-transform duration-700 ease-[0.16,1,0.3,1] group-hover:scale-[1.03]"
